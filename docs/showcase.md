# Showcase: the angloform Linter, end to end
...
- "my"
- "your"
  WORD: "pronouns" is not an angloform word — "Pronouns" is a defined term — write it capitalized (see CONTEXT.md)
...
validated as angloform — quotation-as-mention is a separate, undesigned
...
✓ "the old build process" is a report
  parses uniquely — peak-open 3, max-dep 3, depth 1, right-branching 0%
✓ the agent stores "this is not valid angloform at all !!" in the database
  parses uniquely — peak-open 3, max-dep 4, depth 1, right-branching 33%
```
...
```
✗ Discard the file
  WORD: "Discard" is not an angloform word — a name cannot start a sentence — introduce it ("the tool Lexgen …") or quote it; or if this is a command, use an angloform verb in lowercase ("delete the file")
✓ Lexgen builds the Lexicon
  parses uniquely — peak-open 2, max-dep 2, depth 1, right-branching 33%
✗ the agent reads the File
  WORD: "File" is not an angloform word — angloform words are lowercase ("file"); a name that equals a word needs quotes
```
...
```
✗ it fails
  WORD: "it" is banned in angloform — third-person reference is banned - repeat the noun you mean (ADR 0002)
✓ the test fails
...
✗ please delete the file
  WORD: "please" is banned in angloform — politeness markers are not angloform - state the instruction directly or write "you must ..."
✓ delete the file
  parses uniquely — peak-open 2, max-dep 2, depth 1, right-branching 50%

✗ the agent deleted three files
  WORD: "three" is banned in angloform — counts are digits: write "3 files", not "three files" (ADR 0022)
✓ the agent deleted 3 files
  parses uniquely — peak-open 2, max-dep 2, depth 1, right-branching 25%

✗ the agent deleted 1 file
  WORD: "1" is not an angloform word — write "one" — exactly one stays a word (ADR 0016)
...
✗ the same file fails
  WORD: "same" is banned in angloform — "same" has two meanings - for that very thing, repeat the noun with "the" ("the file", not "the same file"); for two things that match, write "identical" ("the copies are identical") (ADR 0023)
✓ the file fails
  parses uniquely — peak-open 1, max-dep 1, depth 1, right-branching 0%
...
✗ the agent is opening the file
  STYLE:
    - progressive ("is <verb>-ing") is not in angloform — use the simple form (ADR 0003)
✓ the agent opens the file
  parses uniquely — peak-open 2, max-dep 2, depth 1, right-branching 25%

✗ the agent has deleted the file
  STYLE:
    - perfect aspect ("has <verb>-ed") is not in angloform — use the simple past (ADR 0016)
✓ the agent deleted the file
  parses uniquely — peak-open 2, max-dep 2, depth 1, right-branching 25%

✗ the file stored in the database fails
  STYLE:
    - "stored" is transitive in angloform and needs an object
    - this reads as a reduced relative ("the file stored …") — angloform has no relative clauses yet; split into two sentences (ADR 0010)
...
✗ the Seed grows. therefore the expense grows
  WORD: "therefore" is banned in angloform — for a cause and its result write "<cause>, so <result>"; for a result and its reason write "<result>, because <reason>" (ADR 0026)
✓ the Seed grows, so the expense grows
  parses uniquely — peak-open 2, max-dep 3, depth 2, right-branching 14%

✗ the agent stopped
  STYLE:
    - "stopped" is transitive in angloform and needs an object
