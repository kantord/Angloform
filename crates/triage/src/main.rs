...
/// Every OOV (lemma, gold UPOS) pair, ranked by real occurrence count in
/// the corpus — the full list the report's "top 40" only previews.
/// Machine-readable: scripts/candidate-coverage-check.py's migration
/// source of truth (real translation need, not generic word frequency).
const OOV_PATH: &str = "docs/triage-oov-lemmas.tsv";
...
fn main() {
    ...
    std::fs::write(OOV_PATH, render_oov_tsv(&missing_lemmas)).expect("write oov tsv");
    println!(
...
/// The full OOV (lemma, gold UPOS) list, ranked by count descending, as a
/// plain TSV: lemma, upos, count. Same ranking as the report's "top 40"
/// preview, just not truncated.
fn render_oov_tsv(missing_lemmas: &BTreeMap<(String, String), usize>) -> String {
    let mut miss: Vec<(&(String, String), &usize)> = missing_lemmas.iter().collect();
    miss.sort_by(|a, b| b.1.cmp(a.1).then(a.0.cmp(b.0)));
    let mut out = String::from("# generated by triage — do not edit\n# lemma\tupos\tcount\n");
    for ((lemma, upos), n) in miss {
        out.push_str(&format!("{lemma}\t{upos}\t{n}\n"));
    }
    out
}
